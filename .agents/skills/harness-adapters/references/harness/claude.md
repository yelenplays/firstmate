...
The project settings also capture the current transcript at `PreCompact` and `SessionEnd`; both hooks skip Grok and `FM_TASK_ID` workers, while `../../../bin/fm-history.sh` owns privacy filtering and local storage.
