...
# --- ephemeral live driver: pre-owner-era residue is never adopted -----------
test_unowned_legacy_residue_never_adopted_live() {
  local dir state fakebin out capture_file window key pane_hash sig pid seeded_since marker
  dir=$(make_case unowned-legacy-live); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"
  window="test:fm-legacy-live"
  printf 'waiting on ci, no output' > "$capture_file"
  printf 'window=%s\nkind=ship\n' "$window" > "$state/legacy.meta"
  printf 'working: running the long migration step\n' > "$state/legacy.status"
  sig=$(seen_sig "$state/legacy.status"); printf '%s' "$sig" > "$state/.seen-legacy_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "waiting on ci, no output")
  # Pre-owner-era residue: markers under the key with NO .window-owner- record.
  ...
  # REAL watcher, no spawn-claim wrapper: the unowned key is exactly the case.
  ...
  if ! wait_poll_cycle "$state" "$pid" || ! wait_numeric_file "$state/.stale-since-$key" 80; then
    reap "$pid"; fail "an unowned-residue endpoint never reached a fresh stale classification: $(cat "$out")"
  fi
  if ! wait_poll_cycle "$state" "$pid"; then
    reap "$pid"; fail "the unowned-residue successor escalated inside inherited state: $(cat "$out")"
  fi
  grep -F "possible wedge" "$out" >/dev/null \
    && { reap "$pid"; fail "the unowned-residue successor inherited a wedge escalation: $(cat "$out")"; }
  [ "$(cat "$state/.window-owner-$key" 2>/dev/null || true)" = legacy ] \
    || { reap "$pid"; fail "the bind did not claim the unowned key for the live task"; }
  [ ! -e "$state/.wedge-escalations-$key" ] \
    || { reap "$pid"; fail "the bind adopted the ownerless wedge-escalation count"; }
  [ "$(cat "$state/.stale-since-$key")" -gt "$seeded_since" ] \
    || { reap "$pid"; fail "the stale-since timer is the inherited backdated one, not a fresh classification"; }
  reap "$pid"
  unset FM_FAKE_CREW_STATE
  pass "live watcher: a window with no owner record is retired, never adopted"
}
test_unowned_legacy_residue_never_adopted_live
