...
. "/Users/yelen/.no-mistakes/worktrees/548b8aa73bec/01M2X0QG4JHNEJJJXBW86YFAXT/tests/lib.sh"
# shellcheck source=tests/wake-helpers.sh
. "/Users/yelen/.no-mistakes/worktrees/548b8aa73bec/01M2X0QG4JHNEJJJXBW86YFAXT/tests/wake-helpers.sh"
...
set -e
EVIDENCE=/Users/yelen/.no-mistakes/evidence/01M2X0QG4JHNEJJJXBW86YFAXT
fresh_home
add_task ready-1 'Ship GH_TOKEN=synthetic-secret' --kind ship --repo firstmate
add_task held-private 'PRIVATE_CAPTAIN_TITLE' --kind ship --repo firstmate
(cd "$HOME_DIR" && tasks-axi hold held-private --file data/backlog.md --reason PRIVATE_HOLD_REASON --kind captain) >/dev/null
add_task dependency 'Required predecessor' --kind ship --repo firstmate
add_task blocked 'DEPENDENCY_BLOCKED_TITLE' --kind ship --repo firstmate --blocked-by dependency
mkdir -p "$HOME_DIR/data/ready-1"
printf 'PRIVATE_REPORT_BODY\n' > "$HOME_DIR/data/ready-1/report.md"
cp "$HOME_DIR/data/backlog.md" "$TMP_ROOT/before.md"
TYPESAFE_API_KEY=$TS_KEY run_triage code out err --heartbeat
expect_code 0 "$code" 'triage succeeds'
printf '$ fm-jev-queue-triage.sh --heartbeat\n%s\n' "$out"
cp "$LOG/body" "$EVIDENCE/request.json"
cp "$HOME_DIR/state/jev-queue-triage.json" "$EVIDENCE/recommendation.json"
jq -e '(.questions.task.criteria | keys | sort) == ["dependency","none","ready-1"] and .questions.task.criteria["ready-1"] == "Ship [redacted] (ship, firstmate)"' "$LOG/body" >/dev/null
for forbidden in synthetic-secret PRIVATE_CAPTAIN_TITLE PRIVATE_HOLD_REASON PRIVATE_REPORT_BODY DEPENDENCY_BLOCKED_TITLE; do
  assert_not_contains "$(cat "$LOG/body")" "$forbidden" 'private or blocked content excluded'
done
cmp "$TMP_ROOT/before.md" "$HOME_DIR/data/backlog.md"
printf 'Backlog byte-identical after recommendation; held and dependency-blocked items excluded from request.\n'
append_wake "$HOME_DIR/state" heartbeat heartbeat heartbeat
FM_HOME="$HOME_DIR" FM_STATE_OVERRIDE="$HOME_DIR/state" "$DRAIN" > "$EVIDENCE/heartbeat-drain.txt" 2> "$TMP_ROOT/drain.err"
assert_contains "$(cat "$EVIDENCE/heartbeat-drain.txt")" 'task=ready-1' 'recommendation reaches heartbeat display'
printf '$ fm-wake-drain.sh\n'
cat "$EVIDENCE/heartbeat-drain.txt"
jq '.answers.task.confidence=0.1' "$RESPONSE" > "$TMP_ROOT/low.json"
mv "$TMP_ROOT/low.json" "$RESPONSE"
TYPESAFE_API_KEY=$TS_KEY run_triage code out err --heartbeat
printf '$ fm-jev-queue-triage.sh --heartbeat # task confidence 0.1\n%s\n' "$out"
[ ! -e "$HOME_DIR/state/jev-queue-triage.line" ]
cp "$HOME_DIR/state/jev-queue-triage.json" "$EVIDENCE/low-confidence.json"
cmp "$TMP_ROOT/before.md" "$HOME_DIR/data/backlog.md"
printf 'Stale recommendation removed; backlog still byte-identical.\n'
