...
config/claude-remote-control  optional "on [<name>]" opt-in read only by bin/fm-claude-primary.sh to launch a Claude primary with --remote-control for phone steering; absent or "off" launches plain claude; LOCAL, gitignored, and not inherited; see docs/configuration.md "Claude primary Remote Control"
