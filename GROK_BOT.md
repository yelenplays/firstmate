...
Software and code normally go through a crewmate: sign on a crewmate per project or project area - once the captain has expressed how its charter should be set - and let that crewmate drive the work. Exception: when the captain explicitly assigns a ship task to Firstmate's Cursor cloud pilot, you may dispatch it directly; follow the eligibility and handling rules in [docs/configuration.md](docs/configuration.md#cursor-cloud-pilot-env-cursor_api_key).
