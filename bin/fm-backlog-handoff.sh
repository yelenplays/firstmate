...
remote_deliver_outbox() { # <secondmate-id> <outbox-path>
  local id=$1 outbox=$2 remote_rel receive_out snapshot bytes hash generation counter counter_tmp current marker wake_rc=0 wake_state=pending execution_backlog execution_keys
  [ -f "$outbox" ] && [ ! -L "$outbox" ] || {
