...
if [ "$KIND" != secondmate ]; then
  ...
  case "$HARNESS" in
    claude*)
      ...
      # in git, so these hooks are merged in by the script that owns that file's
      # contract; it refuses rather than discarding settings it cannot parse, and
      # records what it added so teardown restores the original bytes. Only a file
      # firstmate itself created is hidden from git's view.
      ...
      claude_hooks_spec="$TASK_TMP/claude-hooks.json"
      cat > "$claude_hooks_spec" <<EOF
{"UserPromptSubmit":"$j_submit","Stop":"$j_stop","StopFailure":"$j_stopfail","SessionEnd":"$j_sessionend"}
EOF
      if ! claude_settings_state=$("$FM_ROOT/bin/fm-claude-worktree-hook.sh" install \
          "$WT" "$TURNEND" "$STATE/$ID.claude-settings-backup" "$claude_hooks_spec"); then
        echo "error: refusing the Claude spawn because the lifecycle hooks could not be added to $WT/.claude/settings.local.json without discarding it; repair that file, then respawn. Inspect target $T" >&2
        exit 1
      fi
