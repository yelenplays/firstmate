...
# The full object is stashed after ingress sanitization, so every conversation-context field the
...
# Only Firstmate's own operational-provenance bytes are removed; an object that
# carried them is stored with fm_provenance_sanitized: true so the drain surfaces
# the attempt, and a poll that cannot run the sanitizer stores nothing.
#
