...
2. **Authorize the replacement worker.**
   A ship or scout relaunch clears its owning home's mandatory Megamind binding first, through a validate-only run of `bin/fm-worker-preflight.sh` that mutates nothing, because the launch owner refuses an unauthorized worker and that refusal would otherwise land after the old agent was already stopped.
   The typed refusal names the exact file to author; [configuration.md](configuration.md#megamind-preflight-configmegamind-) owns the binding contract.
   A secondmate starts a firstmate home rather than an ordinary worker and is not routed through it.
3. **Safe checkpoint.**
...
- `tests/fm-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching, the progress note, the Megamind binding precondition that refuses before the running agent is touched, checkpoint refusals, and rollback after a failed launch.
