...
The last authority consulted before an escalation fires is the task's own no-mistakes run: a crew handed to `axi run` renders a silent pane for the whole server-side validation by design, so pane quietness must never read as a wedge while the run itself proves execution - the 25-consecutive-escalation false alarm reported on a cleanup task whose validation ran the entire time.
`crew_nm_run_progressing` in `bin/fm-classify-lib.sh` owns the proof: bounded `axi status` and overview calls must identify the newest selected run for the task's branch, and its head must exactly match the task worktree unless an active pipeline-owned continuation holds branch custody. A run parked through `awaiting_agent`, `awaiting_approval`, `fix_review`, or any `gate` field is not executing. The run must then show execution evidence - fresh step activity the pipeline itself has not marked quiet, a live non-zombie `agent_pid` or responsive daemon on an in-flight step whose parseable quiet age is below `FM_PAUSE_RESURFACE_SECS`, or a file under `NM_HOME/logs/<run-id>/` written since the pane's quiet window opened (a relative `NM_HOME` resolves from the task worktree).
Any one of those defers the escalation to the same bounded `FM_PAUSE_RESURFACE_SECS` recheck cadence the write deferral uses, with a reason naming the run id rather than a wedge, and it re-arms the idle timer exactly as the sibling deferrals do, so evidence is re-proved once per `FM_STALE_ESCALATE_SECS` instead of trusted once; a quiet process/daemon row stops deferring at the quiet-age bound, while fresh activity or new log writes remain independent evidence.
Every absence of evidence - no recorded worktree, a kind that never validates, a foreign, terminal, or gate-parked run record, a dead agent pid, a failed daemon probe, and a run-log directory nothing has written - leaves the existing escalation schedule untouched, so a task whose run shows neither a live process nor a growing log escalates exactly as before.
...
In away mode, seen-status dedupe does not clear possible-wedge aging for nonterminal progress, so housekeeping still re-escalates an unchanged idle pane at the configured bound, unless the task's own no-mistakes run is demonstrably executing (`crew_nm_run_progressing`), which defers that re-escalation by re-arming the marker exactly as the watcher's run-liveness deferral does.
That deferral has no separate re-surface marker: process/daemon evidence is bounded by the parseable quiet age, and the run's evidence is re-proved at each bound crossing, so a row that stops proving execution still escalates.
