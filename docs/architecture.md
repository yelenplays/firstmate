...
A fleet clone whose origin is a local repository with no remote of its own shares that repository's Treehouse pool, so a fresh ship or scout from the clone refuses before allocation and names the repository to pass instead; [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh)'s `fm_treehouse_pool_origin_root` owns the detection and [`tests/fm-spawn-truth-path.test.sh`](../tests/fm-spawn-truth-path.test.sh) pins it.
