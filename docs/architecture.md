...
A declared clearing time (`paused: ... until <UTC ISO 8601>`) that has already passed stops counting as that account, so a lane whose own wait is over, and a lane that never declared one and has no open captain call, both keep the unchanged escalation schedule, reason and `demand-deep-inspection` wording.
...
A backlog call is instead aged from its own hold-set stamp and throttled under that call's lifecycle identity, so an old status line cannot make a new call re-surface at once and a re-held call never inherits the previous call's silence.
