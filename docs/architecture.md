...
`crew_nm_run_progressing` in `bin/fm-classify-lib.sh` owns the proof: bounded `axi status` and identity-aware run selection identify the task's newest live run, with an anchored runs-ledger fallback for older overview surfaces; its head must exactly match the worktree unless an active pipeline-owned continuation holds branch custody. Terminal or gate-parked runs (`awaiting_agent`, `awaiting_approval`, `fix_review`, or any `gate` field) are not execution evidence. Evidence is any active step with parseable `last_activity` not prefixed by `quiet`; a live non-zombie `agent_pid` or responsive daemon on a quiet active step whose parseable quiet age is below `FM_PAUSE_RESURFACE_SECS`; or a `.log` file under `NM_HOME/logs/<run-id>/` written since the pane's idle anchor. Relative `NM_HOME` resolves from the task worktree.
Accepted evidence re-arms the idle timer, so the watcher re-proves it at each `FM_STALE_ESCALATE_SECS` threshold and re-surfaces the deferral on the `FM_PAUSE_RESURFACE_SECS` cadence instead of trusting it once. Process or daemon evidence for a quiet step expires at the quiet-age bound; parseable non-quiet activity and new log writes remain independent evidence.
Every absence of evidence - no recorded worktree, a kind that never validates, a foreign, terminal, or gate-parked run record, unparseable activity, a dead agent pid, a failed daemon probe, or a run-log directory nothing has written - leaves the existing escalation schedule untouched.
