...
`crew_nm_run_progressing` in `bin/fm-classify-lib.sh` owns the proof: a bounded `axi status` in the task's recorded worktree must answer a still-active run for that task's own branch, and the run must then show execution evidence - fresh step activity the pipeline itself has not marked quiet, a live non-zombie `agent_pid` on an in-flight step, an in-flight daemon-executed step such as the ci monitor while a bounded `daemon status` probe answers, or a file under `~/.no-mistakes/logs/<run-id>/` written since the pane's quiet window opened.
Any one of those defers the escalation to the same bounded `FM_PAUSE_RESURFACE_SECS` recheck cadence the write deferral uses, with a reason naming the run id rather than a wedge, and it re-arms the idle timer exactly as the sibling deferrals do, so evidence is re-proved once per `FM_STALE_ESCALATE_SECS` instead of trusted once.
...
That deferral has no bounded re-surface cadence of its own - unlike the watcher's `.nmrun-resurfaced-` throttle - because adding one would need a new deferral-chain marker in the daemon; the run's own evidence is re-proved at each bound crossing, so a run that stops proving execution still escalates.
