...
A bash watcher (`bin/fm-watch.sh`) sleeps on the fleet, applies deterministic classification, and wakes the first mate only when something is actionable; bounded Jev second opinions can add a surface or defer a would-be wedge alarm at the documented supervision gaps.
...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, free-text or unrecognized status lines escalated by bounded Jev status triage (see [configuration.md](configuration.md#jev-supervision-triage)), authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without a declared wait, a worktree write, an in-bound recorded step, an active no-mistakes run, or a valid negative Jev wedge verdict on an eligible pane tail, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
At the final deterministic stale-escalation check, the task's own no-mistakes run is an additional authority: a crew handed to `axi run` can leave its pane silent throughout server-side validation, so pane silence alone does not prove a wedge.
`crew_nm_run_progressing` in `bin/fm-classify-lib.sh` owns the proof: bounded `axi status` and identity-aware run selection identify the task's newest live run, with an anchored runs-ledger fallback when the overview is unavailable.
The run head must exactly match the worktree unless `branch_sync.state=pipeline_owned` proves active branch custody.
On older CLIs without an overview, an unresolved head is accepted only when the legacy runs ledger ties its active row to the immediately older terminal row at the exact worktree HEAD.
Terminal or gate-parked runs (`awaiting_agent`, `awaiting_approval`, `fix_review`, or any `gate` field) are not execution evidence.
Evidence is any active step with parseable `last_activity` not prefixed by `quiet`; a live non-zombie `agent_pid` or responsive daemon on a quiet active step whose parseable quiet age is below `FM_PAUSE_RESURFACE_SECS`; or a `.log` file under `NM_HOME/logs/<run-id>/` written since the pane's idle anchor.
Relative `NM_HOME` resolves from the task worktree.
Accepted evidence re-arms the idle timer, so the watcher re-proves it at each `FM_STALE_ESCALATE_SECS` threshold and re-surfaces the deferral on the `FM_PAUSE_RESURFACE_SECS` cadence instead of trusting it once.
Process or daemon evidence for a quiet step expires at the quiet-age bound; parseable non-quiet activity and new log writes remain independent evidence.
Unavailable or ambiguous attribution, malformed activity, or any other absence of run evidence leaves the existing escalation schedule untouched.
`tests/fm-watch-triage.test.sh` covers watcher run attribution and evidence boundaries; `tests/fm-daemon.test.sh` covers the matching away-mode deferral.
When those deterministic checks leave an escalation due and an eligible pane tail is available, the watcher and away-mode daemon can make one bounded Jev wedge consult; only a valid negative verdict defers the alarm, while every other outcome preserves structural escalation ([configuration.md](configuration.md#jev-supervision-triage)).
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when every referenced task independently has positive evidence that its crew is still working: a currently attributed active no-mistakes step, or a working verdict from the semantic busy-state contract, both read through `bin/fm-crew-state.sh`.
...
Absorbed wakes advance their suppression markers, log to `state/.watch-triage.log`, and keep the watcher blocking without a queue record or a firstmate turn; the bounded Jev consultations are the narrow in-cycle exception.
...
Only when no matching run exists does it consult semantic busy state; a working verdict reports working, a prompt-waiting Claude verdict reports blocked, exact idle permits fallback to the log's resolved current declaration - the newest decision the fold still holds open, otherwise the latest recognized event - when its verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log.
Decision-only events such as `resolved` never become current state or leak their prose into the current-state detail.
In that status-log fallback, a declared external wait reports the distinct `paused` state with its reason.
The semantic branch reports working only on a working verdict and names the source that produced it; a Claude prompt-waiting verdict reports blocked, and an unknown verdict never becomes working, never permits the status-log fallback, and never becomes a silent idle.
...
The always-on watcher also uses that library's absorb classification on no-verb signals and first-sighting stale panes before status-log terminality is trusted, while the daemon maintains distinct wedge and declared-wait recheck cadences; bounded Jev status triage and the wedge second opinion are detailed in [configuration.md](configuration.md#jev-supervision-triage).
...
In away mode, seen-status dedupe does not clear possible-wedge aging for nonterminal progress; housekeeping uses the run-liveness rule above and re-arms its stale marker when the task's attributed run proves execution, and that run-liveness deferral still has no separate long-cadence re-surface marker.
Away-mode housekeeping has no worktree-write deferral of its own, so writing alone does not defer an alarm; a valid negative Jev wedge verdict may defer it under the separate bounded policy in [configuration.md](configuration.md#jev-supervision-triage).
...
Every classification returns a verdict together with its producing source; the library owns adapter-specific sources, precedence, and trust gates.
...
Ordinary task-state consumers act only on a working verdict, while a Claude session waiting on a prompt answer reports blocked; an unreadable worker surfaces for a closer look instead of being absorbed as still-working or written off as finished.
...
tmux, zellij, orca, and cmux expose no native busy primitive at all, so a task on those backends is classified through the shared adapter-specific sources in `bin/fm-busy-lib.sh`.
...
The pin format, per-id override, and harness/model/effort precedence are owned by [configuration.md](configuration.md#harness-support).
The [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md) owns lifecycle resolution and inherited-local-material propagation; `config/crew-harness` and `config/crew-dispatch.json` remain inherited for a secondmate's own crewmates.
...
A fleet clone refuses a fresh ship or scout before allocation when its plain local-path origin has no `origin` remote and already owns a linked Treehouse pool slot, naming that pool-owning repository to pass instead; [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh)'s `fm_treehouse_pool_origin_root` owns the detection and [`tests/fm-spawn-truth-path.test.sh`](../tests/fm-spawn-truth-path.test.sh) pins it.
