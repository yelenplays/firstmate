...
With no standing declaration, an open captain call in the backlog (`bin/fm-captain-hold.sh open`) is the other record of the same wait and defers the escalation as a hold, because work the captain already holds is never a wedge suspect.
A declared clearing time (`paused: ... until <UTC ISO 8601>`) that has already passed stops counting as a status-line account; if no open captain call is present, a lane whose declared wait has ended or that never declared one keeps the unchanged escalation schedule, reason and `demand-deep-inspection` wording.
...
A backlog call is instead aged from its own hold-set stamp and throttled under that call's lifecycle identity plus the current status-log signature, so an old status line cannot make a new call re-surface at once and a re-held call never inherits the previous call's silence.
...
The wait-evidence check reads the current status line at the at-threshold branch and, when no standing declaration explains the quiet, checks the backlog for an open captain call; it adds no backlog reads to the ordinary poll path.
...
At the final stale-escalation check, the task's own no-mistakes run is an additional authority: a crew handed to `axi run` can leave its pane silent throughout server-side validation, so pane silence alone does not prove a wedge.
...
In away mode, seen-status dedupe does not clear possible-wedge aging for nonterminal progress; housekeeping uses the run-liveness rule above and re-arms its stale marker when the task's attributed run proves execution, but has no separate long-cadence re-surface marker.
