...
`tests/fm-calm-claude-mod-plugin.test.sh` runs wherever `claude` is installed without spending a model turn, on the mods surface's own floor (first verified on 2.1.272): strict `claude plugin validate` on the folder and on the `.claude/skills` auto-load path, then the mod's own `claude plugin test` suites, which drive the hooks module in the engine's host against a mocked clock, environment, file system, and drawing surface. A Claude Code that predates the surface has no `plugin test` command and capability-skips by name; demanding the guard makes that an explicit request failure instead of a silent pass.
...
1. The whole surface is early access and default-off, and its API may change between releases without notice; the real TUI behavior is verified on Claude Code 2.1.272, the plugin compatibility guard also passes on 2.1.273, the mod refuses nothing newer, and `tests/fm-calm-claude-mod-plugin.test.sh` is the check that says when a newer Claude Code stops accepting it. That guard needs the mods surface itself: a `claude` that predates 2.1.272 has no `plugin test`, so it capability-skips by name unless the guard is explicitly requested, where the request fails loudly instead of passing as a skip.
