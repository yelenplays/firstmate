...
`bin/fm-megamind-preflight.sh` descends Megamind's own governed `route` ladder once per authorized full-access root, so an authorization names the pages a routing index points at rather than the index alone; ranked pages must reach the script-owned local relevance floor of 2 before replacing the declared paths, remain bounded by that authorization's own candidate count and character budget, and a ladder that is unusable, fails, or ranks no qualifying page leaves the declared paths exactly as they were.
