...
`bin/fm-megamind-preflight.sh` descends Megamind's own governed `route` ladder once per authorized full-access root, so an authorization names the pages a routing index points at rather than the index alone; the descent asks for `--fields path,kind,score,confidence`, ranked pages must reach the script-owned local relevance floor of 0.5 on Megamind's own per-candidate route-confidence scale before replacing the declared paths, remain bounded by that authorization's own candidate count and character budget, and a ladder that is unusable, fails, reports no per-candidate confidence, or ranks no qualifying page leaves the declared paths exactly as they were.
