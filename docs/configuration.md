...
`config/megamind-primary-automatic` opts the supported Claude, Pi, or pi-signed primary adapter into automatic prompt interception, and unlike its three siblings it is read as a plain switch rather than resolved as a value: its literal first line must be exactly `on`, `true`, `yes`, or `1` and must end in a newline, so a comment header, a blank or whitespace-padded first line, a different case, or a missing final newline all leave the session on its ordinary behavior.
