...
The continuation binds request, preflight, catalog, model class, executable, version, estate identity, date, and current session, serializes consumption, rejects stale or unsafe results, and retires pending evidence only after successful publication.
Its sibling `decline` disposition consumes the same pending offer without invoking Megamind or authorizing or admitting content, and leaves a private replay tombstone so the choice cannot be spent twice.
The date is always the host's own UTC date and the session is always the home's `state/.lock`, so neither an environment token nor the optional `--today` - which may only assert that same date - can forge a binding, and a home with no session lock simply gets its ambiguous result without a continuation handle.
All four accepted releases route the mandatory preflight with the identical proven argv, but `select-offer` and `select-existing` arrive at 0.6.x, so a 0.3.x, 0.4.x, or 0.5.x home keeps its full preflight and gets its ambiguous result without a continuation handle rather than being offered one its build could never honour.
The private store is bounded by the script itself with no daemon: an ambiguous run and a completed continuation or decline retire every pending record bound to another date, drop day-old authorization and no-content disposition tombstones, and keep at most the newest 32 pending records.
...
A matched, no-match, or privacy-filtered result is definitive and lets the spawn continue; an error, unavailable, malformed, or incompatible result blocks it, and `fm-spawn.sh` prints the typed document with the refusal.
An ambiguous result also lets the spawn continue while admitting nothing, because no single confident wiki means the same "load nothing" instruction a no-match carries, and refusing there refused work in exactly the case where the estate had nothing to contribute; the filed result keeps its real outcome so admission refuses it and `state/megamind-preflight.jsonl` still records it as ambiguous, and any offer it carries is left entirely to the primary coordinator below, which alone settles offers.
...
`bin/fm-megamind-preflight.sh` descends Megamind's own governed `route` ladder once per authorized full-access root, so an authorization names the pages a routing index points at rather than the index alone; the descent asks for `--fields path,kind,score,confidence,reasons`, and a ranked page replaces the declared paths only when it satisfies both halves of the script-owned local relevance contract - a floor of 0.5 on Megamind's own per-candidate route confidence, and candidate-local evidence (`index entry match:`) that the page's own index entry is what matched rather than its wiki. Qualifying pages stay bounded by that authorization's own candidate count and character budget, and a ladder that is unusable, fails, reports no per-candidate confidence or reasons, or ranks no page satisfying both halves leaves the declared paths exactly as they were.
...
An `offer` decision reaches only an adapter that can actually put the choice in front of the captain, which today is Pi and pi-signed and their disposition screen below.
For every other adapter the coordinator resolves the ambiguity itself rather than emitting a question that adapter cannot ask: it selects the highest-confidence offer, breaks an exact tie on the wiki name so the same packet always resolves the same way, and continues through the same one-time `select-offer` authorization and bounded admission a captain-driven selection uses.
Every failure on that path - no retained selection, a refused continuation, a failed admission - degrades to the no-wiki disposition rather than to a block, because an automatic resolution may never cost the prompt it was resolving.
For an ambiguous offer, its `continue-no-context` path consumes that one pending offer and returns the exact private request only to an adapter that requested one replay, with no content authorization or admission.
Pi and pi-signed present a custom disposition screen with no initial selection, label candidate wikis as offers, keep Escape as cancel-and-send-nothing, and provide explicit `Continue with no wiki evidence` and `Different existing wiki` actions.
Choosing `Different existing wiki` asks Megamind for the typed bounded eligible-existing list for that exact pending request, model class, owner, session, catalog, and date binding, and renders only the names Megamind returns with no initial selection.
A deliberate `show more` asks Megamind for its bounded full list only when the producer reports truncation; the UI never enumerates roots or invents eligibility.
...
Claude never receives an offer to present, because its transport has no surface that could present one: Claude erases a blocked prompt and shows the block reason to the captain alone and never to the model, so a choice routed there is not a prompt Claude answers but a host control the captain retypes by hand against a request that no longer exists.
Its exact `fm-megamind-select <selection-id> <offer>` and `fm-megamind-none <selection-id>` host controls stay live so a selection retained before the current session is still spendable, and an `offer` that reaches that transport anyway continues the prompt without wiki evidence instead of erasing it.
...
An evidence gap is a forward path rather than an end of turn: the gap is stated plainly, the missing part is researched at a depth proportionate to the request and answered from named sources that stay visibly separate from wiki-grounded material, and the gap is reported back as the owning wiki's - its name, the exact missing topic, and the sources worth ingesting - so the knowledge is compiled through that wiki's own repository and contract rather than written into admitted evidence paths or a guessed wiki location.
The fixed wiki-routing section of every ship and scout brief carries the same rule, so neither an adapter turn that never loaded the `megamind-preflight` skill nor a crewmate in an isolated project copy that cannot load it acts on admitted evidence without the evidence-gap instruction.
...
Admission records remain mode-0600 and reusable through their bound UTC day for relaunch continuity; the date binding refuses rollover, while age-stale records and abandoned artifacts are pruned by the next admission.
...
For Claude crews, `fm-spawn.sh` runs `fm-claude-worktree-hook.sh install`, which merges one Stop hook into the task worktree's `.claude/settings.local.json` beside whatever the project already keeps there, records the pre-existing file so teardown can restore it, and requires `python3`.
It refuses the spawn instead of overwriting settings it cannot read, so a malformed, symlinked, or structurally surprising file is a blocker to repair rather than a silent replacement.
