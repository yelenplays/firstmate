...
`state/.history-cursor` tracks incremental transcript capture, while `state/jev-logbook-highlight.jsonl` and `state/jev-history-find.jsonl` keep metadata-only Jev decision and search records; `bin/fm-history.sh` owns all three.
