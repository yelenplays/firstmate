...
`state/.history-cursor` tracks incremental transcript capture and the latest available token usage for compaction journaling; `state/jev-history-find.jsonl` keeps metadata-only Jev search records. Daily Logbooks and captured wake batches, acknowledgements, and compaction counts live in private `data/history/days/` pages; `bin/fm-history.sh` owns their formats and capture behavior.
