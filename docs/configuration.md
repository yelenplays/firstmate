...
When the optional one-line `$FM_HOME/config/deck-launchd-label` names the Deck's scheduled launchd job, a successful kickstart lets that job rebuild and publish with its own settings; if the job is already running, its 30-minute timer is the backstop. If no usable label is configured or kickstart fails, the checkout's `deploy/refresh.sh work-landed` hook runs directly with this home as the Deck's Firstmate root.
