...
All four accepted releases route the mandatory preflight with the identical proven argv, but `select-offer` arrives at 0.6.x, so a 0.3.x, 0.4.x, or 0.5.x home keeps its full preflight and gets its ambiguous result without a continuation handle rather than being offered one its build could never honour.
...
Pi and pi-signed present a custom disposition screen with no initial selection, label candidate wikis as offers, keep Escape as cancel-and-send-nothing, and provide an explicit `Continue with no wiki evidence` action.
That screen labels `Different existing wiki` and `Propose a new wiki` as unavailable until their typed owners exist; neither action selects an unoffered wiki, enumerates private candidates, or mutates the wiki estate.
