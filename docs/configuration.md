...
The private store is bounded by the script itself with no daemon: an ambiguous run and a completed continuation retire every pending record bound to another date, drop day-old authorization tombstones, and keep at most the newest 32 pending records.
