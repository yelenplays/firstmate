...
That call is hard bounded by `FM_WORKER_PREFLIGHT_TIMEOUT` seconds because both callers run it while holding the home's task-set and per-task spawn locks: expiry is one more typed refusal, so a hung Megamind binary blocks a single launch instead of stalling every other spawn in that home and any forced teardown of it.
...
FM_WORKER_PREFLIGHT_TIMEOUT=60   # seconds allowed for a ship or scout worker's owner-bound Megamind preflight; expiry blocks the launch with a typed failure, and a non-numeric or zero value keeps 60
