...
Its `--help` is its whole interface, flags follow the command, and the header owns the privacy refusal, escalation floor, output, and exit codes. Callers must pass only task facts and never personal data or private-vault content; refusing obvious secrets, email addresses, and phone numbers is only a safety net, not a general personal-data detector.
Ship, scout, and Devin launches carry the spawning home's absolute path as `FM_HOME` and clear inherited provider keys; the key itself is not added to the launch command or environment. Workers must therefore resolve `TYPESAFE_API_KEY` from the firstmate home `.env`; an environment-only key intentionally does not cross the launch boundary.
