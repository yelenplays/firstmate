...
FM_NM_RUN_EVIDENCE_TIMEOUT=10      # wall-clock seconds each bounded `no-mistakes` call in the wedge detector's run-liveness probe may take (`axi status`, plus `daemon status` for a daemon-executed step, plus the run-log mtime read under ~/.no-mistakes/logs/<run-id>/); it runs only at the moment a wedge escalation would otherwise fire, never on every poll, and hitting the bound reads as no run evidence, which leaves the escalation schedule exactly as it was; a value that is not a positive integer falls back to the default
