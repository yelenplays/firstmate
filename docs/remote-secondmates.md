...
After updating a Linux Firstmate checkout that accumulated legacy supervisors, use the worker's `--help` over direct SSH for the supported cleanup procedure; it preserves one healthy current worker and refuses to interrupt a live lane.
