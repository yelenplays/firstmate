...
| `fm-worker-preflight.sh` | Route an ordinary ship or scout brief through its owning home's Megamind binding before launch |
...
| `fm-session-lock-lib.sh` | Shared session-lock identity (ancestry-membership ownership, harness minting, holder liveness) for fm-lock.sh, the Claude Stop auto-arm, and the session-start nudge |
...
| `fm-claude-worktree-hook.sh` | Merge or remove Claude's crew turn-end hook in a worktree's `.claude/settings.local.json` without discarding the project's own settings |
