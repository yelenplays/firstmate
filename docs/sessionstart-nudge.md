...
That predicate treats the recorded pid as an identity and accepts it anywhere in the current ancestry, which keeps a resume or clear silent when the harness has interposed its own helper processes between the hook and the session.
