...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `muse` on their supported backends plus Herdr-only `devin`, while Secondmate spawns cover that same set except the deliberately crewmate-only `muse` and `devin` adapters.
