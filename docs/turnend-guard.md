...
For an in-scope primary, the guard gates on the shared supervision-need predicate in `bin/fm-supervision-lib.sh`: in-flight work counted from `state/*.meta`, registered `state/procevent/*.source` records, or an armed X-mode relay poll at `state/x-watch.check.sh`.
