...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  The same marker guard carries every tracked `.claude/settings.json` entry that must not run under Grok, for one of two reasons.
  Five entries - both `Stop` entries, the `SessionStart` entry, and the two `PreToolUse` Bash entries - are guarded because Grok already covers their event through its own `.grok/hooks/` registration, so an unguarded entry would be a second path.
  The `UserPromptSubmit` entry is guarded for the opposite reason: Grok registers no counterpart, and `bin/fm-claude-primary-prompt.sh` emits only Claude's hook response shape and names its harness as `claude`, while automatic primary Megamind interception is unproven for Grok and reported `unsupported` by `bin/fm-megamind-primary.sh check --harness grok`, so the guard is what keeps a Grok session on ordinary operation instead of handing it a Claude-shaped block or context injection.
  `bin/fm-subagent-pretool-check.sh` is the one deliberate unguarded exception because no Grok registration covers the subagent-spawn event and the check itself is harness-neutral, so guarding it would REMOVE the guard from Grok rather than deduplicate it, recorded in [`subagent-guard.md`](subagent-guard.md) "Known residual gap".
