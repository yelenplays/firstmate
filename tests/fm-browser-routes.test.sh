...
const route = {
  ...
  steps: [
    ...
    { id: 'handback', do: 'handoff', say: 'Continue in the browser', timeoutMs: 20, expect: { appears: { role: 'heading', label: 'Captain completed browser-only step' } } },
  ],
  heal_log: [],
};
...
assert.deepEqual(result.completed, ['name', 'reveal']);
...
assert.equal(result.ok, true);
assert.deepEqual(result.completed, ['reveal']);
assert.deepEqual(result.handoff, { step: 'handback', say: 'Continue in the browser' });
snapshot += '\n  uid=x:5 heading "Captain completed browser-only step"';
result = await engine.executeRoute(route, { name: 'deployment' }, 'handback', page);
assert.equal(result.ok, true);
assert.deepEqual(result.completed, ['handback']);
...
for (const step of [
  { id: 'optional-fill', do: 'fill', target: { role: 'textbox', label: 'Route name' }, value: '${name}' },
  { id: 'optional-select', do: 'select', target: { role: 'combobox', label: 'Route choice' }, option: '${name}' },
]) {
  const optionalRoute = { ...route, vars: { name: { required: false } }, steps: [step] };
  actionLog = [];
  result = await engine.executeRoute(optionalRoute, {}, null, page);
  assert.equal(result.ok, false);
  assert.equal(result.error, 'MISSING_VARIABLE');
  assert.equal(result.step, step.id);
  assert.deepEqual(actionLog, []);
}
const manyStepsRoute = {
  version: 1, host: '127.0.0.1', route: 'many-steps', start: { url_path: '/routes.html' }, vars: {},
  steps: Array.from({ length: 101 }, (_, index) => ({ id: `step-${index}`, do: 'press', key: 'Tab' })),
  heal_log: [],
};
result = await engine.executeRoute(manyStepsRoute, {}, null, page);
assert.equal(result.completed.length, 101);
...
node -e 'const r=JSON.parse(process.argv[1]); if (!r.ok || r.completed.join(",") !== "name,reveal" || r.handoff?.step !== "handoff") process.exit(1)' "$OUT" || fail "route did not return its handoff before waiting: $OUT"
OUT=$(env -u FM_HOME FM_ROOT_OVERRIDE="$TMP_HOME" "$SCRIPT" route run 127.0.0.1/fixture --var name=route-fixture --from handoff --session "$SESSION") || fail "route did not resume after handoff: $OUT"
node -e 'const r=JSON.parse(process.argv[1]); if (!r.ok || r.completed.join(",") !== "handoff" || r.handoff) process.exit(1)' "$OUT" || fail "resumed handoff expectation did not complete: $OUT"
...
LONG_TARGET='Publish this carefully prepared browser route after reviewing every field and confirming all selected destination details'
LONG_EXPECT='The long-label recording fixture confirms the complete action and destination values were recorded without shortening'
OUT=$(FM_HOME="$TMP_HOME" "$SCRIPT" step --click "button=$LONG_TARGET" --expect "heading=$LONG_EXPECT" --record 127.0.0.1/long-label --session "$SESSION") || fail "long-label step was not recorded: $OUT"
node -e 'const r=JSON.parse(require("fs").readFileSync(process.argv[1], "utf8")); if (r.steps[0].target.label !== process.argv[2] || r.steps[0].expect.appears.label !== process.argv[3]) process.exit(1)' "$TMP_HOME/data/browser-routes/127.0.0.1/long-label.json" "$LONG_TARGET" "$LONG_EXPECT" || fail 'recorded labels were truncated'
pass 'recording preserves complete long selector and expectation labels'
...
node - "$TMP_HOME/data/browser-routes/127.0.0.1/many-steps.json" <<'JS'
...
fs.writeFileSync(process.argv[2], `${JSON.stringify(route)}\n`, { mode: 0o600 });
JS
OUT=$(FM_HOME="$TMP_HOME" "$SCRIPT" route run 127.0.0.1/many-steps --session "$SESSION") || fail "long route did not execute: $OUT"
node -e 'const r=JSON.parse(process.argv[1]); if (!r.ok || r.completed.length !== 101) process.exit(1)' "$OUT" || fail 'route output truncated completed steps'
pass 'route output preserves all completed steps'
