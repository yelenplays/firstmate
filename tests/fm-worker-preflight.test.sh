...
EOF
  ...
  expect_code 1 "$rc" "unproven 0.6.0 worker spawn"
  assert_contains "$out" '"code":"version_incompatible"' \
    "unproven 0.6.0 refusal lost its typed failure"
  assert_absent "$home/state/$id.meta" "unproven 0.6.0 published a task record"
